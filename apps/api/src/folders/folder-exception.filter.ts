 ...
 * body. Scoped to FoldersController only via @UseFilters, mirroring
 * MulterExceptionFilter's precedent (apps/api/src/documents/multer-exception.filter.ts)
 * rather than a global filter, since nothing else in this API throws these.
 *
 * Also catches ZodError: every `SomeRequestSchema.parse(body)` call in this
 * codebase (AuthController, DocumentsController, and now this controller)
 * throws a raw, uncaught ZodError on invalid input — there is no global Zod
 * exception handling anywhere in apps/api, so today that surfaces as a 500,
 * not the 400 the "validate every request body" pattern is supposed to
 * produce, and nothing has ever tested the malformed-body path to notice.
 * That's a real, pre-existing, cross-cutting gap, not something new here —
 * fixed for this controller's own routes since it's already building this
 * filter anyway, but deliberately not retrofitted onto AuthController/
 * DocumentsController, which is outside this plan's scope. Flagged in the
 * task report as a recorded-but-deferred finding, same discipline as Task
 * 11's request-validation gap.
 */
