# Infra (Terraform) — ADR-0014 topology (OCI, managed services) — SCALE-UP TARGET, NOT ACTIVE

**Retargeted 2026-08-16 by [ADR-0015](../docs/adr/0015-pre-revenue-single-vm-topology.md).** This is
**not** the active infrastructure and is **not** superseded — it's the topology to apply when there
is revenue and real data volume. The active one is `infra/` (single Always Free VM, $0/month).

**Applying this directory costs ~$240/month**: Container Instances (~$156), OCI Cache with Redis
(~$85), and WAF are **not** Always Free at any size, and the load balancer's `maximum_bandwidth_in_mbps
= 100` puts it outside the free tier's 10 Mbps-min-and-max rule. That contradicts what ADR-0014
originally claimed about the free tier — see the correction in its Status section. Everything here is
`plan`-verified (52 resources, 0 errors) and correct; it is simply the wrong thing to run at zero users.
