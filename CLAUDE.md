...
1. **Start Phase 2** of `docs/plans/implementation-phases-11-07-2026-plan.md` (the main sequential lane, not Phase 2A which is done): folders/groups/permissions (ADR-0005), upload path + serving (ADR-0006/0003), recycle bin. Nothing in this phase has started yet.
2. Phase 2A's UI screens (calendar/kanban board/notification-preferences — spec ready at `docs/ui/calendar-kanban-notifications-addendum-v01.md`) are still unbuilt; can be picked up independently of Phase 2 whenever there's UI bandwidth.
...
6. Phase 0/1 code has still never run against a live Atlas cluster (only Phase 2A's in-memory-Mongo integration suite has exercised real Mongoose queries) — first real live-Atlas run should happen as soon as infra exists
