...
6. Phase 0/1/2 code has now run against a live Atlas cluster for the first time (2026-08-19, via the VM deploy above) — worth a closer look at real query behavior/perf now that this is possible, not just in-memory-Mongo suites. **Atlas Vector Search on M0 is now confirmed working** (2026-08-31, live `$vectorSearch` test against the production cluster — real index build, real query, real cosine-ranked results; full record in ADR-0002's "`$vectorSearch` capability verified live on Atlas M0" section) — `AtlasRetrievalProvider` is mechanically verified, not just written-to-spec. This does **not** touch Hebrew retrieval *quality*, which is still gated on ADR-0008's unrun benchmark corpus (Lane E1) — that remains the real blocker, not cluster capability.
