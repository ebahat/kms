...
### `$vectorSearch` capability verified live on Atlas M0 (2026-08-31)

Whether the free M0 tier actually supports `$vectorSearch` at all was an open, untested question — the document-chat-rag plan's `AtlasRetrievalProvider` was written to this ADR's literal index/query shapes but flagged "unverified against a live cluster" (tracked in the project's root `CLAUDE.md`, not previously called out in this ADR's own text). Resolved by a real, disposable-collection test directly against the production cluster (`cluster0.bnezwpz.mongodb.net`, the same one `deploy/docker-compose.yml` points at):

1. Created a throwaway `_capability_probe._vector_search_probe` collection (never touched real tenant data), inserted 3 tiny 4-dim vectors.
2. Called `collection.createSearchIndex({ type: 'vectorSearch', ... })` via the `mongodb` driver — succeeded (no tier-rejection error).
3. Polled `listSearchIndexes()`: status went `PENDING` → `READY`/`queryable: true` in ~29 seconds — a real async index build, not an immediate no-op.
4. Ran a real `$vectorSearch` aggregation with the literal stage shape this ADR specifies (§"Retrieval query shapes") — got back correctly cosine-ranked results (exact match scored 1.0, near match 0.997, orthogonal 0.5).
5. Dropped the index and collection immediately after.

**Conclusion: M0 supports `$vectorSearch` with no special provisioning or tier upgrade.** This closes the capability question — it does **not** touch Hebrew retrieval *quality* against real documents, which stays exactly where ADR-0008 left it: gated on Lane E1's benchmark corpus, not yet run. `AtlasRetrievalProvider` can now be considered mechanically verified, not just written-to-spec; live quality validation is the remaining gap.

A separate real-document measurement the same day (parsing 8 sample files from `test-datasets/` through the actual `libs/parsing` chunker) found real Hebrew protocol PDFs producing roughly **9.7 chunks per MB of source file**, ~9KB of Atlas storage per chunk (dominated by the 768-dim embedding vector, not the text) — materially sparser than this ADR's own "~100 chunks/doc" planning-stage assumption for typical short documents, though that ratio will vary with document density (scanned/image-heavy PDFs contribute file bytes without a proportional increase in extractable text or chunks). Useful input for capacity-planning M0's 512MB cap against a specific tenant's real document mix, not a change to any index design decision here.
