...
**Revisited 2026-08-02** (owner asked whether `me-west1`/Tel Aviv is viable, prompted by MongoDB Atlas offering it as a cluster region): **rejected, region stays europe-west.** MongoDB Atlas and core GCP services (Cloud Run, Memorystore) do support `me-west1` (Atlas: M10+ only, no Free/Flex tier there) — but Vertex AI's documented generative-AI data-residency guarantee only covers North America, Europe, and Asia regions; Google's own docs state that for any regional endpoint outside that table (Middle East included), there is no guarantee ML processing happens in a specific location. Splitting the topology — DB/cache/compute in Tel Aviv, Gemini chat/embedding calls landing wherever Vertex actually serves them — would add real cross-region complexity and latency on the most user-facing path (chat) while *losing* the clean single-region residency story for the most sensitive data class (document content in prompts/embeddings). Worse on both axes than staying put. Revisit if Vertex AI ever publishes a Middle East data-residency region; no other trigger.
