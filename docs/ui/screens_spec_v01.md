...
5. B2 tree reachability when a permission-widened subfolder's only access path is a direct/shared link and no ancestor is visible to the user (PRD §7 narrowed open question) — needs a resolved UX (synthetic path breadcrumb? direct-link-only, no tree entry?) before B2 is implemented. The widening badge + group-visibility requirement itself (PRD §7 "Permission-widening indicator") is settled and not blocked by this.
