...
* **Directory browsing and navigation:** Users can browse the full directory tree and see the list of subfolders and files in each directory they open, navigating (drilling down / "changing directory") into any directory where they hold at least read access, whether via a direct grant, group grant, inheritance from a parent, or public-folder status. A directory the user has no access to does not appear anywhere in the tree or in a parent's listing — it is omitted outright, not shown as locked or forbidden, so its existence can't be inferred by browsing (no existence oracle, consistent with the 404-not-403 policy applied elsewhere, sec §3.2).
* **Permission-widening indicator:** A subfolder whose permission grants have been overridden to reach a wider audience than its parent folder — e.g., a group added that the parent doesn't grant, or the folder made public when its parent isn't — is visibly marked in the folder browser and document list as having broader-than-parent access. Any user who can already read that subfolder can see which **groups** have access to it (not just tenant admins), so they can judge whether it's appropriate to store something sensitive there before doing so, without needing to ask an admin. This is a read-time transparency affordance, not a grant-editing capability — changing grants still requires `manage` tier (§7 tiers). Individually-granted users are not named in this view (only in the tenant-admin permissions screen) to avoid exposing one person's personal grant to every other viewer of the folder.
* **Open question (narrowed):** if a user's only path to a permission-widened subfolder is a direct/shared link — no ancestor folder in the chain is itself visible to them — the tree-navigation mechanics for reaching that folder by browsing (as opposed to via the link) are still undecided. The widening indicator and group-visibility requirement above apply regardless of how the folder was reached.
